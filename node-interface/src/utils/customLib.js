...
module.exports = {
	...
	log: function(title) {
		if (console && console.log) {
			...
			if (typeof title == 'string') {
				console.log(title + ':' + (this.time - last) / 1000);
			}
		}
	},
	//type 文件类型
	getContentType:function(type) {
		let contentTypeArr ={
			...
			'js': 'application/ecmascript', //		相当于': 'application/javascript但是严格的处理规则
			...
			'js': 'application/javascript', //		ECMAScript/JavaScript（相当于': 'application/ecmascript但是宽松的处理规则）
			...
			'woff': 'application/font-woff', //		Web开放字体格式（Web Open Font Format，简称WOFF）是一种网页所采用的字体格式标准。（推荐使用；使用': 'application/x-font-woff直到它变为官方标准）

  ...
  $.fn.extend({
    phoneSelect: function(options) {
        ...
        function setPhoneCode(){
            var setPhoneSelectData = function(phoneSelectData){
                var $tabHead = $('<div class="phone_prefix_tab"></div>');
                var $tabContent = $('<div class="tab-content"></div>');
                $.each(phoneSelectData,function(i,group){
            ...
            var phoneSelectData = Prefs.Get('phoneSelectData')
            if(phoneSelectData && phoneSelectData.length>0){
                setPhoneSelectData(phoneSelectData)
            }else{
                $.ajax({
                    type: "get",
                    ...
                    success: function(data) {
                        Prefs.Set('phoneSelectData',data)
                        setPhoneSelectData(data)
                    }
                });
            }
        }

...
import { remove, getList, save } from '@/api/lpm/knowledgepledge'
...
export default {
  ...
  data() {
    return {
      formVisible: false,
      formTitle: '添加知识产权出质登记信息',
      propertyTypeList: [], // 种类，从数据字典中获取
      ...
      rules: {
        ...
        propertyRegisterCode: [
          { required: true, message: '请填写知识产权登记证号', trigger: 'blur' }
        ],
        propertyName: [
        ...
        pledgorName: [
          { required: true, message: '请填写出质人名称', trigger: 'blur' }
        ],
        pledgeeName: [
          { required: true, message: '请填写质权人名称', trigger: 'blur' }
        ],
        pledgeeCertificateNumber: [
          { required: true, message: '请填写质权登记期限', trigger: 'blur' }
        ],
        status: [
  ...
  methods: {
    ...
    fetchData() {
      ...
      dictList({ name: '种类【知识产权出质登记信息】' }).then(response => {
        this.propertyTypeList = getDictList(response.data[0].detail)
      })
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          save({
            ...
            propertyRegisterCode: this.form.propertyRegisterCode,
            propertyName: this.form.propertyName,
            propertyType: this.form.propertyType,
            pledgorName: this.form.pledgorName,
            pledgeeName: this.form.pledgeeName,

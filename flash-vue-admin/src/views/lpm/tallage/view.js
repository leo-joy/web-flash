...
import { getList as tallage } from '@/api/lpm/tallage'
...
export default {
  ...
  data() {
    return {
      id: '',
      /* 商标信息模块 */
      tallageData: [], // 商标信息相关数据
      levyType: '', // 类别
      accessoryFilesListTallage: [],
      listQuery: {
        page: 1,
        limit: 20,
        ids: ''
  ...
  methods: {
    init() {
      ...
      /* 商标信息模块 */
      tallage({ enterpriseCode: id, page: 1, limit: 20 }).then(response => {
        this.tallageData = response.data.records
        var accessoryArr = ['accessoryFiles']
        this.getFilesList('Tallage', accessoryArr, response.data.records)
      })
    },

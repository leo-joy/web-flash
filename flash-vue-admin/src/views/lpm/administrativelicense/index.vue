  ...
  <div class="app-container">
    <div class="block">
      <el-row>
        <el-col :span="24">
          <el-button
            v-permission="['/administrativelicense/add']"
            type="success"
            icon="el-icon-plus"
            size="small"
            @click.native="add"
          ...
          <el-button
            v-permission="['/administrativelicense/edit']"
            type="primary"
            icon="el-icon-edit"
            size="small"
            @click.native="edit"
          ...
          <el-button
            v-permission="['/administrativelicense/delete']"
            type="danger"
            icon="el-icon-delete"
            size="small"
            @click.native="remove"
    ...
    >
      ...
      <el-table-column label="许可文件名称" width="300">
        <template slot-scope="scope">{{ scope.row.permissionFileName }}</template>
      </el-table-column>
      <el-table-column label="有效期自" width="150">
        <template slot-scope="scope">{{ scope.row.validityFrom.replace(' 00:00:00','') }}</template>
      </el-table-column>
      <el-table-column label="有效日期至" width="150">
        <template slot-scope="scope">{{ scope.row.validityEnd.replace(' 00:00:00','') }}</template>
      </el-table-column>
      <el-table-column label="许可机关" width="300">
        <template slot-scope="scope">{{ scope.row.permissionOrg }}</template>
      </el-table-column>
      <el-table-column prop="status" label="状态" width="150" :formatter="formatterLicenseStatus" />
    ...
    <el-dialog :title="formTitle" :visible.sync="formVisible" width="70%">
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="许可机关" prop="permissionOrg">
              <el-input v-model="form.permissionOrg" minlength="1" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="许可内容" prop="permissionContent">
              <el-input v-model="form.permissionContent" type="textarea" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script src="./administrative_license.js"></script>

  ...
  <div class="app-container">
    <div class="block">
      <el-row>
        <el-col :span="24">
          <el-button
            v-permission="['/propertypledge/add']"
            type="success"
            icon="el-icon-plus"
            size="small"
            @click.native="add"
          ...
          <el-button
            v-permission="['/propertypledge/edit']"
            type="primary"
            icon="el-icon-edit"
            size="small"
            @click.native="edit"
          ...
          <el-button
            v-permission="['/propertypledge/delete']"
            type="danger"
            icon="el-icon-delete"
            size="small"
            @click.native="remove"
    ...
    >
      ...
      <el-table-column label="登记日期" width="150">
        <template slot-scope="scope">{{ scope.row.registerDate.replace(' 00:00:00','') }}</template>
      </el-table-column>
      <el-table-column label="登记机关" width="300">
        <template slot-scope="scope">{{ scope.row.registerOrg }}</template>
      </el-table-column>
      <el-table-column label="被担保债权数额" width="200">
        <template slot-scope="scope">{{ scope.row.byAssureBondContribution }}</template>
      </el-table-column>
      <el-table-column prop="status" label="状态" width="150" :formatter="formatterPropertyStatus" />
...
<script src="./property_pledge.js"></script>

  ...
  computed: {
    ...
    rules() {
      return {
        ...
        shareType: [
          { required: true, message: '请选择股份类型', trigger: 'blur' }
        ],
        status: [
  ...
  methods: {
    ...
    fetchData() {
      ...
      dictList({ name: '类型【股东信息】' }).then(response => {
        this.shareholderTypeList = getDictList(response.data[0].detail)
      })

      dictList({ name: '股份类型【股东信息】' }).then(response => {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          save({
            ...
            numberOfPaidShares: this.form.numberOfPaidShares,
    ...
    createStateFilter(queryString) {
      return (state) => {
        ...
        console.log(state.enterpriseName)
        console.log(queryString)
        console.log(state.enterpriseName.indexOf(queryString) === 0)
